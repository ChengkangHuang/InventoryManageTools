...
public class MainActivity extends AppCompatActivity {
    ...
    // TODO: Button listener needs to be optimized
    ...
    public void setButtonStatus(View gridChild, ItemRequest item, boolean isEnable, int color) {
        ...
        plusImageButton.setVisibility(View.GONE);
        ...
        sendButton.setOnClickListener(v -> {
            myRef.child(item.getItemName()).child("empty").setValue(true);
            Toast.makeText(this, "Order sent > " + item.getItemName(), Toast.LENGTH_SHORT).show();
            Vibrator vibrator = (Vibrator) getSystemService(Context.VIBRATOR_SERVICE);
            vibrator.vibrate(VibrationEffect.createOneShot(150, VibrationEffect.DEFAULT_AMPLITUDE));
        });
        sendButton.startAnimation(AnimationUtils.loadAnimation(this, R.anim.shake));
    }
        ...
        switch (Objects.requireNonNull(snapshot.getKey())) {
            case "empty":
                if ((boolean) Objects.requireNonNull(snapshot.getValue())) {
                    item.setEmpty(true);
                    setItemStatus(gridViewChild, "Ordered", Color.parseColor("#FFC107"));
                    ...
                    item.setEmpty(false);
                    setItemStatus(gridViewChild, "Ready to Order", Color.parseColor("#4CAF50"));
                    setButtonStatus(gridViewChild, item, true, Color.parseColor("#03A9F4"));
                }
                break;
            case "quantity":
